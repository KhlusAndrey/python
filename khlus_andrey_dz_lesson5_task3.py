...
tutors2 = [
    'Иван', 'Анастасия', 'Петр', 'Сергей', 'Дмитрий', 'Борис', 'Елена', 'Петр', 'Сергей', 'Дмитрий',
]


def gen_tutors(tutors, klasses):
    if len(tutors) > len(klasses):
        for tut_tup in itertools.zip_longest(tutors, klasses, fillvalue=None):
            print(tut_tup)
    else:
        for tut_tup in zip(tutors, klasses):
            print(tut_tup)


gen_tutors(tutors, klasses)
gen_tutors(tutors2, klasses)
